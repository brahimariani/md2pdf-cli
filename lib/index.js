...
// (heading ids, KaTeX extension) never leaks across invocations.
function renderMarkdown({ md, math, toc, tocDepth, tocTitle }) {
  const m = new Marked();
  ...
  const headings = collectHeadings(m.lexer(md));
...
async function convert(options) {
  ...
  const { body: parsedBody, tocHtml } = renderMarkdown({
    md,
    math,
    toc,
    tocDepth,
  ...
  const fullBody = tocHtml ? `${tocHtml}\n${parsedBody}` : parsedBody;
